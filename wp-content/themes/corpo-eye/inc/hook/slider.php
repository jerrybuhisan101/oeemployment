...
// Check slider status.
add_filter( 'corpo_eye_filter_slider_status', 'corpo_eye_check_slider_status' );

// Add slider to the theme.
add_action( 'corpo_eye_action_before_content', 'corpo_eye_add_featured_slider', 5 );

// Slider details.
add_filter( 'corpo_eye_filter_slider_details', 'corpo_eye_get_slider_details' );


if ( ! function_exists( 'corpo_eye_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function corpo_eye_get_slider_details( $input ) {

		$featured_slider_type           = corpo_eye_get_option( 'featured_slider_type' );
		$featured_slider_number         = corpo_eye_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = corpo_eye_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = corpo_eye_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'corpo-eye-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = corpo_eye_the_excerpt( apply_filters( 'corpo_eye_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'corpo_eye_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function corpo_eye_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'corpo_eye_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'corpo_eye_filter_slider_details', $slider_details );
		...
		// Render slider now.
		corpo_eye_render_featured_slider( $slider_details );

	}
endif;


if ( ! function_exists( 'corpo_eye_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function corpo_eye_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}
		$featured_slider_transition_effect   = corpo_eye_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = corpo_eye_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = corpo_eye_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = corpo_eye_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = corpo_eye_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = corpo_eye_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = corpo_eye_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = corpo_eye_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'corpo_eye_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function corpo_eye_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = corpo_eye_get_option( 'featured_slider_status' );
